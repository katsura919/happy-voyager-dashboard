 ...
 */
export async function fetchPostCountsByCategory(): Promise<
...
/**
 * Fetch blog post stats: published vs draft counts, and monthly post creation counts.
 */
export async function fetchBlogPostStats(): Promise<{
  published: number;
  draft: number;
  ...
  return { published, draft, monthlyCounts };
}

/**
 * Fetch comment stats: pending, approved, and spam counts.
 */
export async function fetchCommentStats(): Promise<{
  ...
  for (const comment of data || []) {
    ...
    else if (comment.status === "approved") approved++;
    else if (comment.status === "spam") spam++;
  }

  return { pending, approved, spam };
}

/**
 * Fetch top blog posts ordered by views.
 */
export async function fetchTopBlogsByViews(
  limit = 8,
): Promise<
  Pick<BlogPost, "id" | "title" | "slug" | "views" | "status" | "category">[]
> {
  const supabase = createClient();
  const { data, error } = await supabase
    .from("blog_posts")
    .select("id, title, slug, views, status, category")
  ...
  return (data ?? []) as Pick<
    BlogPost,
    "id" | "title" | "slug" | "views" | "status" | "category"
  >[];
}
